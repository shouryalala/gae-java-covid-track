...
public class ListBookServlet extends HttpServlet {
  ...
  public void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    try {
      Result<Record> result = dao.listRecords(startCursor);
      logger.log(Level.INFO, "Retrieved list of all records");
      records = result.getResult();
      endCursor = result.getCursor();
    } catch (Exception e) {
      throw new ServletException("Error listing records", e);
    }
    req.getSession().getServletContext().setAttribute("records", records);
    StringBuilder recordNames = new StringBuilder();
    for (Record record : records) {
      recordNames.append(record.getName()).append(" ");
    }
    logger.log(Level.INFO, "Loaded books: " + recordNames.toString());
